...
class m110810_162301_userTimestampFix extends CDbMigration
{
	public function safeUp()
	{
        ...
        switch ($this->dbType()) {
            case "mysql":
                    $this->addColumn(Yii::app()->getModule('user')->tableUsers,'created_at',"TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP");
                    $this->addColumn(Yii::app()->getModule('user')->tableUsers,'lastvisit_at',"TIMESTAMP NOT NULL DEFAULT '0000-00-00 00:00:00'");
                    $this->execute("UPDATE ".Yii::app()->getModule('user')->tableUsers." SET created_at = FROM_UNIXTIME(createtime), lastvisit_at = IF(lastvisit,FROM_UNIXTIME(lastvisit),'0000-00-00 00:00:00')");
                    $this->dropColumn(Yii::app()->getModule('user')->tableUsers,'createtime');
                    $this->dropColumn(Yii::app()->getModule('user')->tableUsers,'lastvisit');
                break;
            case "sqlite":
            default:
                    $this->addColumn(Yii::app()->getModule('user')->tableUsers,'created_at',"TIMESTAMP");
                    $this->addColumn(Yii::app()->getModule('user')->tableUsers,'lastvisit_at',"TIMESTAMP");
                    $this->execute("UPDATE ".Yii::app()->getModule('user')->tableUsers." SET created_at = datetime(createtime, 'unixepoch'), lastvisit_at = datetime(lastvisit, 'unixepoch')");
                    ...
                    $this->execute('INSERT INTO "'.Yii::app()->getModule('user')->tableUsers.'" SELECT "id","username","password","email","activkey","superuser","status","created_at","lastvisit_at" FROM "'.__CLASS__.'_'.Yii::app()->getModule('user')->tableUsers.'"');
	...
	public function safeDown()
	{
        ...
        switch ($this->dbType()) {
            case "mysql":
                ...
                $this->execute("UPDATE ".Yii::app()->getModule('user')->tableUsers." SET createtime = UNIX_TIMESTAMP(created_at), lastvisit = UNIX_TIMESTAMP(lastvisit_at)");
                $this->dropColumn(Yii::app()->getModule('user')->tableUsers,'created_at');
            ...
            default:
                ...
                $this->execute("UPDATE ".Yii::app()->getModule('user')->tableUsers." SET createtime = strftime('%s',created_at), lastvisit = strftime('%s',lastvisit_at)");
                $this->execute('ALTER TABLE "'.Yii::app()->getModule('user')->tableUsers.'" RENAME TO "'.__CLASS__.'_'.Yii::app()->getModule('user')->tableUsers.'"');
                $this->createTable(Yii::app()->getModule('user')->tableUsers, array(
                    "id" => "pk",
                    "username" => "varchar(20) NOT NULL",
