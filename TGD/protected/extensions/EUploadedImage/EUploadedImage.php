...
//	private $_thumb_prefix = 'thumb_';
	...
	public function saveAs($file)
	{
		if($this->_error==UPLOAD_ERR_OK)
		{
			...
			if ($this->saveImage($file)) {
				...
				if ($this->thumb) {
                    $this->saveFormatImage($file, $this->thumb);
				}
                // create the preview
                if ($this->preview) {
                    $this->saveFormatImage($file, $this->preview);
                }
                return true;
			}
		}
     ...
     */
    private function saveFormatImage($file, $data)
    {
        // create the thumbnail of the requested dimensions
        $format_max_width = isset($data['maxWidth']) ? $data['maxWidth'] : false;
        $format_max_height = isset($data['maxHeight']) ? $data['maxHeight'] : false;
        $this->imagePrepare($format_max_width, $format_max_height);
        // get the thumb folder
        $format_dir = isset($data['dir']) ? $data['dir'].DIRECTORY_SEPARATOR : NULL;
        // get the thumb prefix
        $format_prefix = isset($data['prefix']) ? $data['prefix'] : NULL;
        // save the thumbnail
        $format_file = substr_replace($file, $format_dir.$format_prefix, strrpos($file, DIRECTORY_SEPARATOR) + 1, 0);

        $formats_dir =  substr($format_file, 0, strrpos($format_file, DIRECTORY_SEPARATOR));

        if(!is_dir($formats_dir)){
            mkdir($formats_dir, 0777);
        }

        return $this->saveImage($format_file);
    }
